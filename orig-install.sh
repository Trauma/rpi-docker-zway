...
FILE=`basename z-way-server/z-way-server-RaspberryPiXTools-v2.2.2-test-new-webserver-0-g000000.tgz`
if [[ -e $TEMP_DIR/$FILE ]]; then
	echo "Removing duplicate of z-way installer"
	rm -rf $TEMP_DIR/$FILE
fi
echo "Getting Z-Way for Raspberry Pi and installing"
wget -4 http://razberry.z-wave.me/z-way-server/z-way-server-RaspberryPiXTools-v2.2.2-test-new-webserver-0-g000000.tgz -P $TEMP_DIR/
...
mkdir -p /etc/z-way
echo "v2.2.2-test-new-webserver-0-g000000" > /etc/z-way/VERSION
...
# Stop readKey if exist
if [ -f /etc/init.d/readKey ];then
	/etc/init.d/readKey stop
fi

# Create readKey startup script
echo "Creating readKey startup script"
echo '#! /bin/bash
### BEGIN INIT INFO
# Provides:		  readKey
# Required-Start:
...
#For any questions contact sts@zwave.eu (Stefan Schreiter)
#14.05.2016 version 1.0
...
exit 0' > /etc/init.d/readKey
chmod +x /etc/init.d/readKey

# Add readKey to autostart
echo "Adding readKey to autostart"
###update-rc.d readKey defaults

# Starting readKey mongoose
###/etc/init.d/readKey start
...
if [[ $RPI_BOARD_REVISION ==  "a02082" ]]
then
	echo "Raspberry Pi 3 Detected. Disabling Bluetooth"
	# Remove "console=serial0,115200" from /tmp/zway_install_cmdline.txt
	sed -i 's/console=serial0,115200//' /tmp/zway_install_cmdline.txt
